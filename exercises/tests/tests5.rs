...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must ensure that the address is valid and points to
    // a mutable reference to a `u32` value. Additionally, the function must
    // only be called with a unique reference to the memory location.
    unsafe {
        // Your code goes here
